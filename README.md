...
## 🕐 New: Daily Scheduler System
This project now includes a **fully self-contained daily scheduler** that automatically runs the script at a configured time each day, without requiring external dependencies like cron or Task Scheduler.

**Quick Setup:**
```bash
# Enable scheduler to run daily at 6:00 AM UTC
...
📚 **[Complete Scheduler Documentation](SCHEDULER.md)**
...
- **NEW**: For daily automation, use the built-in scheduler instead of external cron jobs or Task Scheduler
- If you automate this script manually, set it to run at least 2 times a day to make sure it picked up all tasks, set `"runOnZeroPoints": false` so it doesn't run when no points are found.
...
|  dailyRetries     | Additional retries after initial run (0 = no retries, 2 = 3 total passes) | `0` |
|  retryDelayMinutes     | Minutes to wait between daily retry runs | `30` |
